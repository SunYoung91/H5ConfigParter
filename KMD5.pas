...
uses
    ...
    function StreamToMD5(Stream:TMemoryStream):String;
...
begin
    S := AnsiString(M);
    MD5Init(Context);
    MD5Update(Context, Pointer(S), length(S));
...
begin
  MD5Init(Context);
  PMem := Stream.Memory;
  Inc(PMem,Stream.Position);
  MD5Update(Context,PMem,Stream.Size - Stream.Position);
  MD5Final(Context, MD5);
  Result := UpperCase(MD5Print(MD5));
end;
end.
